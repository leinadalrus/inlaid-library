...
void virtual_device_information_set_irq(VirtualDeviceInformation *dev_info, int irq) {
...
uint64_t virtual_device_information_read(void *any_opaque, hwaddr address_offset unsigned int data_size) {
  uint64_t ret_val = 0;
...
uint64_t virtual_device_information_write(void *any_opaque, hwaddr address_offset, uint64_t assigned_value, unsigned int data_size) {
  uint64_t ret_val = 0;
  VirtualDeviceInformation *dev_info = (VirtualDeviceInformation*) any_opaque;
  DecryptedCommandBufferTree *command_buffer_tree;
  int is_enabled = dev_info->virtual_device_data->destination & BIT(0);
